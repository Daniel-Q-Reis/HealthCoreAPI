                ...
                <div className="container mx-auto px-4">
                    <div className="grid md:grid-cols-2 gap-8">
                        {resources.map((res, idx) => (
                            <div key={idx} className="bg-white p-8 rounded-xl shadow-md border border-gray-100 hover:shadow-lg transition flex gap-6 items-start">
                                <div className="bg-gray-50 p-4 rounded-full text-3xl shrink-0">
                                    {res.icon}
                                </div>
                                <div>
                                    <h3 className="text-xl font-bold text-[#003B5C] mb-2">{res.title}</h3>
                                    <p className="text-gray-600 mb-4">{res.desc}</p>
                                    <a href="#" className="text-[#0066CC] font-semibold hover:underline">Learn More →</a>
                                </div>
                            </div>
                        ))}
                    </div>
                    ...
                    {/* Newsletter */}
                    <div className="mt-16 bg-[#003B5C] rounded-2xl p-12 text-center text-white relative overflow-hidden">
                        <div className="relative z-10 max-w-2xl mx-auto">
                            <h2 className="text-3xl font-bold mb-4">Stay Healthy, Stay Informed</h2>
                            <p className="text-blue-100 mb-8">Subscribe to our monthly newsletter for the latest health news and hospital updates.</p>
                            <div className="flex flex-col md:flex-row gap-4 justify-center">
                                <input
                                    type="email"
                                    placeholder="Enter your email address"
                                    className="px-6 py-3 rounded-lg text-gray-900 w-full md:w-96 focus:outline-none focus:ring-2 focus:ring-[#FFD100]"
                                />
